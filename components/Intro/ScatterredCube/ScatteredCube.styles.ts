...
const ba4gon = `360deg / ${n4gon}`
const tu = `2 * ${t} / ${Math.pow(m, 3)}` // delay unit
...
export const Assembly = styled(Container)`
  top: 50%;
  left: 50%;
  transform: rotateX(-22.5deg) rotateY(-30deg);
`

export const Cube = styled(Container)`
  ...
  ${(props) => {
    ...
    for (let i0 = 0; i0 < nc; i0++) {
      ...
      const xi = `${f[0]} * ${j0} * ${l}` // initial x coord
      const xf = `${f[1]} * ${xi}` // final x coord
      const r = `${nc - i0 - 0.5} * ${hu}` // red component

      for (let i1 = 0; i1 < nc; i1++) {
        const j1 = i1 - m
        const k1 = Math.pow(j1, 2)
        const yi = `${f[0]} * ${j1} * ${l}` // initial y coord
        const yf = `${f[1]} * ${yi}` // final y coord
        const g = `${i1 + 0.5} * ${hu}` // green component

        for (let i2 = 0; i2 < nc; i2++) {
          const j2 = i2 - m
          const k2 = Math.pow(j2, 2)
          const zi = `${f[0]} * ${j2} * ${l}` // initial z coord
          const zf = `${f[1]} * ${zi}` // final z coord
  ...
  ${(props) => {
    ...
    for (let i = 0; i < nf; i++) {
      styles += `
        &:nth-child(${i + 1}) {
          transform: ${
            i < n4gon ? `rotateY(${i} * ${ba4gon})` : `rotateX(Math.pow(-1, ${i}) * ${ba4gon})`
          } translateZ(0.5 * ${l});
        }
      `
    }
